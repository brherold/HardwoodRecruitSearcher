...
#USE WHEN FLIP OCCURED AND PLAYER TABLES ARE NOT UPDATED FOR THEIR CURRENT YEAR
...
def recruitSearchFunction(wantedYear, wantedRegion, recruited, developmentDiff, min_potential, min_si, min_height, min_wingspan, min_vertical,
    ...
    for player in playerLinks:
        ...
        #When Tables aren't updated for player
        '''
        1 year behind checking, if checking JR, do for dev check for FR-SO
        '''
    ...
    ###############################################################################################################    
        ...
        if wantedYear not in ["INT","JCFR","JCSO"]:
            ...
            #Table not updated, for Sophmore use this
            ...
            #gets current height not from stats Table
            curr_height = float(convert_to_inches(soup2.find("table").find_all("tr")[6].find("td").text.split("  ")[-1]))
